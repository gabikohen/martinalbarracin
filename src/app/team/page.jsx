...
export default function NuestroEquipo() {
  return (
    <section id="nuestroEquipo" aria-labelledby="ne-title" className="w-full min-h-screen py-20 px-6 bg-black text-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-lg sm:text-xl mb-16 leading-relaxed text-gray-100 max-w-3xl mx-auto text-center">
          Somos un equipo <span style={GREEN_FOOTER_GLOW} className="font-semibold">apasionado por crear experiencias únicas y hacer que tu diversión llegue al siguiente nivel. </span> 
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
          {teamMembers.map((member, index) => (
            <motion.div
              ...
              className="p-4 sm:p-6 md:p-7 lg:p-8 rounded-xl bg-transparent backdrop-blur-md border border-white/20 shadow-2xl hover:border-white/40 transition-all"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.1, duration: 0.5 }}
              whileHover={{ boxShadow: "0px 20px 35px rgba(51, 140, 59, 0.3)" }}
              viewport={{ once: true }}
            >
              <div className="w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 lg:w-40 lg:h-40 mx-auto mb-4 rounded-full border-2 border-green-500/50 overflow-hidden">
                <img
                  src={member.image}
              ...
              <h3 className="text-lg sm:text-xl font-bold mb-2 text-center" style={GREEN_FOOTER_GLOW}>{member.name}</h3>
              <p className="text-sm sm:text-base md:text-lg font-semibold text-gray-300 mb-2 sm:mb-3 text-center">{member.role}</p>
              {member.bio && <p className="text-xs sm:text-sm text-gray-400 text-center leading-relaxed">{member.bio}</p>}
            </motion.div>
          ))}
        </div>
      </div>
